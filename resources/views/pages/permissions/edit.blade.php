                                    ...
                                    @foreach($roles as $role)
                                        <div class="relative flex gap-x-3">
                                            <div class="flex h-6 items-center">
                                                <input
                                                    class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600"
                                                    {{ $permission->roles->contains('id', $role->id) ? 'checked' : '' }}
                                                    value="{{$role->id}}" id="role-{{$role->id}}" name="role_id[]" type="checkbox"/>
                                            </div>
