            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex w-full mb-3">
                        <form action="{{ route('users.index') }}" method="GET" @submit.prevent="$event"
                        ...
                        <tbody class="w-full border-b px-3 text-center px-6 py-6">
                            @forelse($users as $user)
                                <tr class="border-b">
                                    ...
                                    <td>
                                        <span
                                            class="ms-3 text-sm font-bold border-2 rounded-lg px-3 py-1 {{ $user->status->value === 1 ? 'text-green-600 dark:text-green-500 border-green-600 dark:border-green-500' : 'text-red-600 dark:text-red-500 border-red-600 dark:border-red-500'}}">
                                            {{ __($user->status->name) }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="flex gap-3 justify-center">
                                        ...
                                        @if ($user->status->value == 1)
                                            <form action="{{ route('users.destroy', $user->id) }}" method="POST" @click="$event.preventDefault()">
                                                @csrf
                                                @method('DELETE')
                                                <x-secondary-button type="submit">
                                                            <span class="material-icons text-xs">
                                        ...
                                        @else
                                            <form action="{{ route('users.restore', $user->id) }}" method="POST" @click="$event.preventDefault()">
                                                @csrf
                                                @method('PUT')
                                                <x-secondary-button type="submit">
                                                            <span class="material-icons text-xs">
